...
RSpec.describe Crm::Onboarding::Missing::Day4 do
  ...
  context "when user has locale set and not onboarded" do
    ...
    let(:expected_text) { I18n.t("crm.onboarding.missing.day4.text") }
    let(:expected_button_text) { I18n.t("crm.onboarding.missing.day4.button") }
    ...
    context "when day1 notification is not sent" do
      it { is_expected.not_to be_executable }
    end

    context "when day1 notification sent" do
      before { create(:crm_notification, crm_type: Crm::Onboarding::Missing::Day1.name, user: user, created_at: 5.days.ago) }
