...
RSpec.describe Crm::Arena::Champion::TopUp::Day28 do
  ...
  context "when user's balance is zero" do
    ...
    let(:expected_text) { I18n.t("crm.arena.champion.top_up.day28.text") }
    let(:expected_button_text) { I18n.t("crm.arena.champion.top_up.day28.button") }
    ...
    context "when day21 notification not sent" do
      it { is_expected.not_to be_executable }
    end

    context "when day21 notification sent" do
      before do
        create(:crm_notification,
          crm_type: Crm::Arena::Champion::TopUp::Day21.name,
          user: user,
