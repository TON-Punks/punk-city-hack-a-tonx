...
RSpec.describe BlackMarket::PurchaseEmojiPack do
  ...
  let!(:seller) { create(:user, chat_id: 1351881907) }

  let(:payment_amount) { product_price }

  before do
    allow(BlackMarketProduct).to receive(:find_by).with(slug: BlackMarketProduct::EMOJI_PACK).and_return(product)
  ...
  context 'when pay method is praxis' do
    ...
    specify do
      ...
      expect(seller.praxis_balance).to eq(1000)
