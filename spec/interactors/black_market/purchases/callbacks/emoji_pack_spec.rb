...
RSpec.describe BlackMarket::Purchases::Callbacks::EmojiPack do
  ...
  let(:animated_punk_url) { 'https://link.com/punk.mp4' }

  let(:payment_amount) { product_price }
  let(:notification_message) do
    I18n.t("admin.notifications.emoji_pack_purchased.common",
  ...
  before do
    allow(punk).to receive(:animated_punk_url).and_return(animated_punk_url)
